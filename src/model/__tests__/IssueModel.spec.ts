...
import { Issue, countIssuesByStatus, getAssignedToName, getFormattedDate, getTimeLeft } from '../Issue';
...
describe('Issue Class', () => {
  test('creates an instance of Issue with the provided properties', () => {
    ...
    const createdBy = {id: 34, firstName: 'John', lastName: 'Doe', assignedIssues: []};
    const createdAt = new Date();
    const closedAt = null;
    const dueTo = new Date();
    const status = Status.Open;

    const issue = new Issue(id, name, description, assignedTo, createdBy, createdAt, closedAt, dueTo, status);
...
describe('getAssignedToName', () => {
  test('returns the assigned name when assignedTo is not null', () => {
    const issue = new Issue(1, 'Test Issue', null, {id: 34, firstName: 'John', lastName: 'Doe', assignedIssues: []}, {id: 4245, firstName: 'Jane', lastName: 'Smith', assignedIssues: []}, new Date(), null, null, null);
  ...
  test('returns an empty string when assignedTo is null', () => {
    const issue = new Issue(1, 'Test Issue', null, null, {id: 4245, firstName: 'Jane', lastName: 'Smith', assignedIssues: []}, new Date(), null, null, null);
    const assignedToName = getAssignedToName(issue);
...
describe('getFormattedDate', () => {
  test('returns a formatted date string when dueTo is not null', () => {
    ...
    const issue = new Issue(1, 'Test Issue', null, null, {id: 4245, firstName: 'Jane', lastName: 'Smith', assignedIssues: []}, new Date(), null, dueTo, null);
...
describe('getTimeLeft', () => {
    ...
    test('returns 0 when dueTo is in the past', () => {
      const currentTime = new Date('2023-07-01').getTime();
  ...
  describe('countIssuesByStatus', () => {
    const issueList = [
      new Issue(1, 'Issue 1', null, null, {id: 4245, firstName: 'Jane', lastName: 'Smith', assignedIssues: []}, new Date(), null, null, Status.Open),
      new Issue(2, 'Issue 2', null, null, {id: 34, firstName: 'John', lastName: 'Doe', assignedIssues: []}, new Date(), null, null, Status.Closed),
      new Issue(3, 'Issue 3', null, null, {id: 4245, firstName: 'Jane', lastName: 'Smith', assignedIssues: []}, new Date(), null, null, Status.InProgress),
      new Issue(4, 'Issue 4', null, null, {id: 34, firstName: 'John', lastName: 'Doe', assignedIssues: []}, new Date(), null, null, Status.Open),
      new Issue(5, 'Issue 5', null, null, {id: 4245, firstName: 'Jane', lastName: 'Smith', assignedIssues: []}, new Date(), null, null, Status.Closed),
    ];
